...
reducePointsToPlot = T # reduce number of points by random selection (needed for pdf output as vectorgraphics will take to much space otherwise). The procedure will only filter out SNPs in dense areas and low p-values
...
plots = foreach(
  file = file_list,
  .packages = c("data.table", "stringr", "CMplot")
) %dopar% {
