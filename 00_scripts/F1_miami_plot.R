...
data_name = paste0("plot_data_all_gw_hits_publication", Sys.Date(), ".RData")
# data_name = "plot_data_all_gw_hits_publication_2024-05-07.RData"
...
filter_max = T # check whether filtering for max values
...
novelty = c(T, T, T, T, F, F, F, T, F, T, T) # locus 7 is set to known as we do not want to highlight the loci that are not independent
...
# add cumulative position for x-axis labeling
plot_data = plot_data %>%
  group_by(chrom) %>%
...
plot_combined = ggplot() +
  geom_point(
    data = all_data[pval <= -log10(5 * 10^-8)],
    aes(
      x = rel_pos, y = pval,
      col = color
    ),
  ...
  scale_y_continuous(
    limits = c(2, maxp + 2), # add 2 for label positioning
...
plot_females = ggplot() +
  geom_point(
    data = female_data[pval <= -log10(5 * 10^-8)],
    aes(
      x = rel_pos, y = pval,
      col = color
    ),
...
plot_males = ggplot() +
  geom_point(
    data = male_data[pval <= -log10(5 * 10^-8)],
    aes(
      x = rel_pos, y = pval,
      col = color
    ),
  ...
  scale_y_reverse(
    limits = c(maxp + 2, 2), # add 2 for label positioning
    expand = expansion(mult = c(0.02, 0.02))
  ) +
  geom_hline(
    yintercept = -log10(5e-8),
...
dummy_legend_plot = ggplot() +
  geom_point(
    data = dummy_dat[pval <= -log10(5 * 10^-8)],
    aes(
      x = rel_pos, y = pval,
      col = trait
    ),
