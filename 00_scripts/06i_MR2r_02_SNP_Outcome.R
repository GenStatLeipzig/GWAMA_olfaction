#' ---
#' title: "Mendelian Randomization 4 - extract SNP effects on OP"
...
#' # Introduction ####
#' ***
#' This is the second script for MR4. Here, I will get the SNP - outcome association (AD/PD --> OP). The SNP list has been generated in the previous script (15_MR4_01_GetInstruments.R)
#'
#' # Initialize ####
#' ***
#' pipeline_name: 15_MR4_02_SNP_Outcome.R
...
ToDoList = data.table(
  phenotype = statistics3[seq(1, length(statistics3), 2)],
...
dumTab1 = foreach(i = 1:dim(ToDoList)[1]) %do% {
  ...
  erg1[, diff := abs(EAF - myTab2$EAF_UKBB)]
  erg1 = erg1[diff < 0.2]
  ...
  # save in temp file
  save(erg1, file = paste0(path_MR_temp, "MR4_", myRow$phenotype, "_", myRow$setting, "_filtered.RData"))
  erg1
}
