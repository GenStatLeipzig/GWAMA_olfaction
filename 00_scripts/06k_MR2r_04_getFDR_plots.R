...
#' # Introduction ####
#' ***
#' This is the forth MR script for MR4. Here, I will load the results, and perform FDR by exposure and setting.
#'
...
dumTab1 = foreach(i = 1:length(mySettings)) %do% {
  ...
  myFDR = addHierarchFDR(
    pvalues = MRTab2[, pval_IVW],
...
for (sex_setting in mySettings) {
  plotData[setting == sex_setting, pval_adjusted := p.adjust(pval_IVW, method = "fdr"), by = outcome]
}
plotData = plotData[pval_adjusted < 0.05]
plotData = plotData[outcome %in% c("SCORE"), ] # restrict to outcomes for which an influence is supported by literature
plotData = plotData[exposure %in% c("ALZHEIMER", "AD_WIDE", "PARKINSON"), ]
...
ToDoList = data.table(
  outcome = rep(myOutcomes, each = 9),
  exposure = rep(myExposures, 3),
  setting = rep(c("female", "male", "all"), each = 3)
)
