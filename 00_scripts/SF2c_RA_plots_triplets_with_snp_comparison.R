...
phenotype_data = foreach(
  file = pheno_files_list,
...
layout(matrix(c(1, 1, 2, 3), nrow = 2, byrow = T))
  ...
  for (myPheno in myPhenos) {
    ...
    points(
      x = locus[snp == top_snp_a, position], y = -log10(locus[snp == top_snp_a, pvalue]),
      pch = 21, col = "black", cex = 3, lw = 3
    ) # not used for region 6 because it is identical to the top-SNP
    points(
      x = locus[snp == top_snp_m, position], y = -log10(locus[snp == top_snp_m, pvalue]),
      pch = 21, col = "green", cex = 3, lw = 3
    )
    points(
      x = locus[snp == top_snp_f, position], y = -log10(locus[snp == top_snp_f, pvalue]),
