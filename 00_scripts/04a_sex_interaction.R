...
#' # Introduction ####
#' ***
#' Perform sex interaction testing for index variants.
#'
#' # Initialize ####
#' ***
...
for (phenotype in phenotype_list) {
  ...
  # no genome-ide significant locus can be found
  ...
  write.table(
    df,
    file = paste0("11_sex_interaction/sex_ia_", phenotype, ".txt"),
...
# SNPs that are not significant after FDR correction are not labelled
...
if (p_threshold == 5e-8) {
  ...
  # row with test for score in region 8 is removed so only the top-phenotype is tested
  ...
  myPlotData[meandiff_p < 0.05, sig := "yes"] # SNPs that are not sig. after FDR correction are considered not significant
  ...
  locus_def = fread(paste0(path_locus_definition, "locus_definition_rsID.csv"),
    dec = ","
  ) # TODO check locus definition file with rsIDs
  ...
  myPlot1 = ggplot(
    ...
    geom_errorbarh(
      aes(xmin = beta_male - 1.96 * se_male, xmax = beta_male + 1.96 *
        se_male),
      height = 0.02
    ) +
...
# Unlabeled version for suggestive hits -----------------------------------
...
if (p_threshold != 5e-8) {
  ...
  png(
    filename = paste0("11_sex_interaction/BetaBeta_sexIA_suggestive.png"),
    width = 6000, height = 6000, res = 600
  )
  print(myPlot1)
  dev.off()
