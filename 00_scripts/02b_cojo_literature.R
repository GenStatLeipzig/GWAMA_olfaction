...
novelty_file = "additional_information/novelty_status.csv" # manually created table of intersection between defined loci and literature
...
for (row in 1:nrow(novelty)) {
  ...
  phenotypes = region_data$phenotype
  ...
  for (phenotype in phenotypes) {
    ...
    # select lit snp to condition on
    ...
    cojo_input = data_tmp[
      (chrom == lit_chrom) & (pos >= region_start) & (pos <= region_end),
