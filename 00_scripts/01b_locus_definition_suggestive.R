...
#' # Introduction ####
#' ***
#' Definition of the genome wide significant 
#'
#' # Initialize ####
#' ***
#' Update server & set working directory
# pipeline_name: 05a_locus_definition_with_suggestive_hits.R
...
#'
#' 1. Step: get genome-wide significant and suggestive SNPs of all phenotypes and settings
...
result.2 = foreach(s = subsets) %do% {
  ...
  while (smallestDist < 500000) {
    ...
    if(length(myPos) > 1){
      message("WARNING: snp ", myPos[1], "not unique.\n")
      myPos = myPos[1] #select one for script to run
    }
...
#add information about genomewide significance
result.4[pFEM <= 5*10^-8, genomewide_sig := T]
result.4[pFEM > 5*10^-8, genomewide_sig := F]
...
write.table(
  result.4,
  file = paste0(path_locus_definition, "locus_definition_suggestive.csv"),
  col.names = T,
  row.names = F,
  quote = F,
  sep = ";",
...
#Outdated, quality metrics are now included in the formatted file
# #get the imputation quality score
# 
# message("\n--------------------------\n")
# message("Loading files for imputation quality...\n")
# 
# chroms_to_load=unique(result.4$CHROM)
# 
# 
# info_data = foreach (chrom = chroms_to_load) %do% {
#   if(chrom==23) chrom = "X"
#   fn = paste0(path_imputation_info, "chr_", chrom,"/chr", chrom, ".info.gz")
#   info_file = fread(fn, nThread = 30)
#   info_file
# }
# 
# names(info_data) = as.character(chroms_to_load)
# 
# impu_qual_column = c()
# for (row in c(1:nrow(result.4))){
#   rsq = info_data[[as.character(result.4[row, CHROM])]][SNP==result.4[row,MarkerName], Rsq]
#   if (length(rsq)==0){
#     rsq = NA
#   }
#   impu_qual_column = c(impu_qual_column, rsq)
# }
# 
# result.4[,"Life Rsq" := impu_qual_column]
# 
# write.table(
#   result.4,
#   file = paste0(path_locus_definition, "locus_definition_all_phenotypes_with_life_rsq_V2.csv"),
#   col.names = T,
#   row.names = F,
#   quote = F,
#   sep = ";",
